...
function createMessageTextures(scene) {
    ...
    const phishingTexture = scene.textures.createCanvas('phishing-message', 300, 100);
...
function ensureScenesLoaded() {
    ...
    const requiredScenes = ['BootScene', 'MenuScene', 'GameScene', 'GameOverScene'];
    ...
    for (const sceneName of requiredScenes) {
        if (typeof window[sceneName] !== 'function') {
            ...
            window[sceneName] = class extends Phaser.Scene {
                ...
                create() {
                    this.add.text(400, 300, `${sceneName} Not Found`, {
                        fontSize: '32px',
                        fill: '#ff0000'
                    }).setOrigin(0.5);
                    
                    this.add.text(400, 350, 'Please check browser console for errors', {
                        fontSize: '16px',
                        fill: '#ffffff'
                    }).setOrigin(0.5);
