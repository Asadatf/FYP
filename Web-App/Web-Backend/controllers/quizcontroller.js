  ...
  try {
    ...
    if (!title || !difficulty || questions === undefined || !timeEstimate) {
      return res.status(400).json({
        error: 'Missing required fields. Title, difficulty, questions, and time estimate are required.'
      });
    }
  ...
  "questions": [
    ...
    const fallbackQuiz = {
      ...
      questions: Array.from({ length: req.body.numberOfQuestions }, (_, i) => ({
        ...
        correctAnswer: Math.floor(Math.random() * 4),
        explanation: `This is the explanation for question ${i + 1} about ${req.body.topic}.`
      })),
...
export const validate_quiz = async (req, res) => {
  try {
    ...
    if (!quizData || !userAnswers) {
      return res.status(400).json({ error: "Quiz data and user answers are required" });
    }
